...
<div id="status-event-area">
    ...
    if ($current_status == 1): ?>
        <button onclick="toggleShopEvent(0)" class="btn btn-success">
            <i class="fas fa-check-circle"></i> Shop đang tham gia
        </button>
    <?php else: ?>
        <button onclick="toggleShopEvent(1)" class="btn btn-secondary">
            <i class="fas fa-times-circle"></i> Shop đã tắt tham gia
        </button>
    <?php endif; ?>
</div>
<script>
function toggleShopEvent(status) {
    if(!confirm("Bạn có chắc chắn muốn thay đổi trạng thái tham gia sự kiện cho TOÀN BỘ sản phẩm?")) return;
    
    $.post("update_shop_event.php", { status: status }, function(res) {
        if(res === "OK") {
            location.reload(); // Load lại để cập nhật nút
        } else {
            alert("Có lỗi xảy ra!");
        }
    });
