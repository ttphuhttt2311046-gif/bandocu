...
/* ======================================================
   1.5) NẠP TRẠNG THÁI SỰ KIỆN VÀO SESSION (NẾU CHƯA CÓ)
====================================================== */
if (!isset($_SESSION['shopThamGiaSuKien'])) {
    $stmtUser = $conn->prepare("SELECT shopThamGiaSuKien FROM taikhoan WHERE maTaiKhoan = ?");
    $stmtUser->bind_param("i", $user_id);
    $stmtUser->execute();
    $resUser = $stmtUser->get_result()->fetch_assoc();
    
    // Nếu tìm thấy thì lưu vào session, mặc định là 1 nếu không thấy
    $_SESSION['shopThamGiaSuKien'] = $resUser['shopThamGiaSuKien'] ?? 1;
}
/* ======================================================
...
<div id="status-event-area">
    <?php 
    // Lấy trực tiếp từ DB để đảm bảo nút luôn đúng với thực tế cơ sở dữ liệu
    $sql_check = "SELECT shopThamGiaSuKien FROM taikhoan WHERE maTaiKhoan = " . intval($_SESSION['user_id']);
    $res_check = $conn->query($sql_check);
    $row_check = $res_check->fetch_assoc();
    $current_status = $row_check['shopThamGiaSuKien'] ?? 1;

    if ($current_status == 1): ?>
        <button onclick="toggleShopEvent(0)" class="btn btn-success">
            <i class="fas fa-check-circle"></i> Hiện đang tham gia sự kiện
        </button>
    <?php else: ?>
        <button onclick="toggleShopEvent(1)" class="btn btn-secondary">
            <i class="fas fa-times-circle"></i> Hiện không tham gia sự kiện
        </button>
    <?php endif; ?>
</div>
<script>
function toggleShopEvent(status) {
    // 1. Kiểm tra xem hàm có nhận lệnh không
    console.log("Hàm đã gọi với status:", status);
    
    if(!confirm("Bạn có chắc chắn muốn thay đổi?")) return;
    
    // 2. Thực hiện gọi AJAX
    $.ajax({
        url: "update_shop_event.php",
        type: "POST",
        data: { status: status },
        // Chèn thêm dòng này để đảm bảo không bị cache
        cache: false,
        success: function(res) {
            ...
            if(res.trim() === "OK") {
                ...
                alert("Lỗi từ file xử lý: " + res);
            }
        },
        error: function(xhr, status, error) {
            console.error("Lỗi AJAX chi tiết:", xhr.responseText);
            alert("Lỗi kết nối (Mã: " + xhr.status + "). Vui lòng kiểm tra Console.");
        }
    });
}
</script>
