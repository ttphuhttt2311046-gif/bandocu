...
if (isset($_GET['toggle_event'])) {
    ...
    $action = $_GET['action']; 
    // Nếu action là 'tuchoi' -> set thamGiaSuKien = 0
    // Nếu action là 'thamgia' -> set thamGiaSuKien = 1
    $val = ($action === 'thamgia') ? 1 : 0;
    
    $conn->query("UPDATE sanpham SET thamGiaSuKien = $val WHERE maSanPham = $id");
    exit("OK");
}
/* ============================================================
...
if (isset($_POST['list'])) {
    ...
    $sql = "SELECT 
    s.maSanPham, s.tenSanPham, s.gia, s.giamGia, s.thamGiaSuKien,
