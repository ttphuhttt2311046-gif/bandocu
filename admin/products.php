...
loadProducts();
function toggleEvent(id, action) {
    let msg = action === 'tuchoi' ? "Bạn muốn từ chối tham gia sự kiện Admin?" : "Bạn muốn tham gia lại sự kiện Admin?";
    if (!confirm(msg)) return;

    $.get("products_manager.php", { 
        toggle_event: id, 
        action: action // truyền 'tuchoi' hoặc 'thamgia'
    }, function(res) {
        if (res === "OK") {
            loadProducts(); 
        } else {
