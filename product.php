...
if ($product) {
    $giaGoc = $product['gia'];
    $giamCaNhan = intval($product['giamGia'] ?? 0); 
    $giamSuKien = 0;
    
    // Lấy thời gian hiện tại
    $now = date('Y-m-d H:i:s');
    
    // Câu lệnh SQL lấy sự kiện đang diễn ra
    ...
    if ($eventRes && $eventRes->num_rows > 0) {
        ...
        // Kiểm tra điều kiện tham gia sự kiện của sản phẩm
        // Lưu ý: SQL của bạn dùng cột 'thamGiaSuKien'
        if (isset($product['thamGiaSuKien']) && (int)$product['thamGiaSuKien'] === 1) {
...
<div class="price-container" style="line-height: 1.4;">
    <?php if ($giamApDung > 0): ?>
        <div class="old-price" style="text-decoration: line-through; color: #999; font-size: 16px;">
            <?= number_format($product['gia'], 0, ',', '.'); ?> ₫
        </div>
        
        <div class="new-price-row" style="display: flex; align-items: center; gap: 10px;">
            <span style="color: #e74c3c; font-size: 26px; font-weight: bold;">
                <?= number_format($giaMoi, 0, ',', '.'); ?> ₫
            </span>
            <span style="background: #e74c3c; color: white; padding: 2px 8px; border-radius: 4px; font-size: 14px; font-weight: bold;">
                -<?= $giamApDung; ?>%
            </span>
        </div>
