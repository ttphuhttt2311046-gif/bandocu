...
<!-- 🔻 BANNER -->
<div class="banner-wrapper"><!-- thêm wrapper -->
  ...
  <!-- controls đặt ngoài phần chạy -->
  <div class="banner-controls">
    <button id="banner-prev" aria-label="Prev">◀</button>
    <button id="banner-next" aria-label="Next">▶</button>
  </div>
</div>

<!-- phần JS chỉ thay đổi chút selector -->
<script>
document.addEventListener('DOMContentLoaded', function(){
    const wrapper = document.querySelector('.banner-wrapper');
    const container = wrapper.querySelector('.banner-img1'); // element trượt
    const slider    = container.querySelector('.banner-slider');
    if (!wrapper || !container || !slider) return;
    ...
    function centerSlide(idx){
        ...
        const it = items[idx];
        const target = it.offsetLeft + it.clientWidth/2 - container.clientWidth/2;
    ...
    // **mở lại khi click bất kỳ chỗ nào ngoài banner**
